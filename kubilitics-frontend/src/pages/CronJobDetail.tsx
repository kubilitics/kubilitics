...
export default function CronJobDetail() {
  ...
  // These useMemo hooks MUST be called before any early return to satisfy Rules of Hooks.
  // Moving them after conditional returns causes "Rendered more hooks" errors.
        ...
        <SectionCard icon={Terminal} title="Terminal" tooltip={<p className="text-xs text-muted-foreground">Exec into CronJob Job pods</p>}>
          {cjPods.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <PodTerminal podName={terminalPod} namespace={namespace ?? undefined} containerName={selectedTerminalContainer || terminalPodContainers[0]} containers={terminalPodContainers} onContainerChange={setSelectedTerminalContainer} />
            </div>
          )}
        </SectionCard>
      ),

...
export default function ServiceAccounts() {
 ...
 const patchSA = usePatchK8sResource('serviceaccounts');
 ...
 const allItemKeys = useMemo(() => itemsOnPage.map((sa) => `${sa.namespace}/${sa.name}`), [itemsOnPage]);

 const toggleSelection = (sa: ServiceAccount, event?: React.MouseEvent) => {
 ...
 return executeBulkOperation(Array.from(selectedItems), async (_key, ns, name) => {
 await patchSA.mutateAsync({ name, namespace: ns, patch: { metadata: { labels: { [labelKey]: labelValue } } } });
 });
 };
 ...
 <ListPageStatCard label="Total" value={stats.total} icon={UserCircle} iconColor="text-primary" selected={!hasActiveFilters} onClick={clearAllFilters} className={cn(!hasActiveFilters && !isLoading && 'ring-2 ring-primary')} isLoading={isLoading} />
 <ListPageStatCard label="With Tokens" value={stats.withTokens} icon={UserCircle} iconColor="text-emerald-600" valueClassName="text-emerald-600" selected={columnFilters.hasTokens?.size === 1 && columnFilters.hasTokens.has('Yes')} onClick={() => setColumnFilter('hasTokens', new Set(['Yes']))} className={cn(columnFilters.hasTokens?.size === 1 && columnFilters.hasTokens.has('Yes') && 'ring-2 ring-emerald-500')} isLoading={isLoading} />
 <ListPageStatCard label="With Image Pull Secrets" value={stats.withImagePullSecrets} icon={UserCircle} iconColor="text-blue-500" valueClassName="text-blue-500" selected={columnFilters.hasImagePullSecrets?.size === 1 && columnFilters.hasImagePullSecrets.has('Yes')} onClick={() => setColumnFilter('hasImagePullSecrets', new Set(['Yes']))} className={cn(columnFilters.hasImagePullSecrets?.size === 1 && columnFilters.hasImagePullSecrets.has('Yes') && 'ring-2 ring-blue-500')} isLoading={isLoading} />
 <ListPageStatCard label="Default" value={stats.defaultNamed} icon={UserCircle} iconColor="text-muted-foreground" title="Named &quot;default&quot;" isLoading={isLoading} />
 </div>
 ...
 <ListPageLoadingShell columnCount={13} resourceName="service accounts" isLoading={isLoading} onRetry={() => refetch()} />
 ) : isError ? (
 <TableRow>
 ...
 <tr key={`${sa.namespace}/${sa.name}`} className={cn(resourceTableRowClassName, idx % 2 === 1 && 'bg-muted/5', selectedItems.has(`${sa.namespace}/${sa.name}`) && 'bg-primary/5')}>
 <TableCell onClick={(e) => { e.stopPropagation(); toggleSelection(sa, e); }}><Checkbox checked={selectedItems.has(`${sa.namespace}/${sa.name}`)} tabIndex={-1} aria-label={`Select ${sa.name}`} /></TableCell>

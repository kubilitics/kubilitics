...
export default function ResourceQuotas() {
 ...
 const { data, isLoading, isError, refetch, pagination: hookPagination } = usePaginatedResourceList<ResourceQuotaResource>('resourcequotas');
 ...
 const patchRQ = usePatchK8sResource('resourcequotas');
 ...
 const toggleSelection = (r: ResourceQuotaRow, event?: React.MouseEvent) => {
 ...
 return executeBulkOperation(Array.from(selectedItems), async (_key, ns, name) => {
 await patchRQ.mutateAsync({ name, namespace: ns, patch: { metadata: { labels: { [labelKey]: labelValue } } } });
 });
 };
 ...
 <ListPageLoadingShell columnCount={13} resourceName="resource quotas" isLoading={isLoading} onRetry={() => refetch()} />
 ) : isError ? (
 <TableRow>

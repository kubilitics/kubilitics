...
const JOBS_COLUMNS_FOR_VISIBILITY = [
 ...
 { id: 'backoffLimit', label: 'Backoff Limit' },
...
export default function Jobs() {
 ...
 const patchResource = usePatchK8sResource('jobs');
 ...
 columns: [
 { columnId: 'name', getValue: (j: Job) => j.name, sortable: true, filterable: true },
 ...
 tableId: 'jobs',
 columns: JOBS_COLUMNS_FOR_VISIBILITY,
 alwaysVisible: ['name'],
 });
...
`,
 ...
 const toggleSelection = (item: Job, event?: React.MouseEvent) => {
 ...
 return executeBulkOperation(Array.from(selectedItems), async (_key, ns, name) => {
 await patchResource.mutateAsync({ name, namespace: ns, patch: { metadata: { labels: { [labelKey]: labelValue } } } });
 });
 };
 ...
 <TableHead className="w-10"><Checkbox checked={isAllSelected} onCheckedChange={toggleAll} aria-label="Select all jobs" className={cn(isSomeSelected && 'data-[state=checked]:bg-primary/50')} /></TableHead>
 ...
 <ListPageLoadingShell columnCount={16} resourceName="jobs" isLoading={isLoading} onRetry={() => refetch()} />
 ) : isError ? (
 <TableRow>
 ...
 <TableCell>
 <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="press-effect h-8 w-8 text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors" aria-label="Job actions"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
 <DropdownMenuContent align="end" className="w-48">
 <CopyNameDropdownItem name={item.name} namespace={item.namespace} />
 <DropdownMenuItem onClick={() => navigate(`/jobs/${item.namespace}/${item.name}`)} className="press-effect gap-2">View Details</DropdownMenuItem>

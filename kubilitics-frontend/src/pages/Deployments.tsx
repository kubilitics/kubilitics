...
interface DeploymentResource extends KubernetesResource {
 ...
 status: { replicas?: number; readyReplicas?: number; updatedReplicas?: number; availableReplicas?: number; conditions?: Array<{ type: string; status: string; reason?: string; message?: string }> };
...
interface Deployment {
 ...
 /** Human-readable reason for the current status (derived from K8s conditions) */
 statusReason: string;
 ready: string;
 upToDate: number;
 available: number;
...
function transformResource(resource: DeploymentResource): Deployment {
 ...
 // Derive human-readable reason from K8s conditions
 let statusReason = '';
 if (status === 'Progressing') {
 const progressCond = conditions.find((c) => c.type === 'Progressing');
 const availCond = conditions.find((c) => c.type === 'Available');
 if (progressCond?.reason === 'ProgressDeadlineExceeded') {
 status = 'Degraded'; // timed out = degraded, not just progressing
 statusReason = progressCond.message || 'Deployment progress deadline exceeded';
 } else if (progressCond?.message) {
 statusReason = progressCond.message;
 } else if (availCond?.status === 'False' && availCond?.message) {
 statusReason = availCond.message;
 } else {
 statusReason = `Waiting for ${desired - ready} pod${desired - ready > 1 ? 's' : ''} to become ready`;
 }
 ...
 statusReason = 'No replicas are ready';
 }
 }
...
export default function Deployments() {
 ...
 const { data, isLoading, isError, isFetching, dataUpdatedAt, refetch } = useK8sResourceList<DeploymentResource>('deployments', undefined, { limit: 5000 });
...
`,
 ...
 <ListPageLoadingShell columnCount={visibleColumnCount} resourceName="deployments" isLoading={isLoading} onRetry={() => refetch()} />
 ) : isError ? (
 <TableRow>

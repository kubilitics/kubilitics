...
export default function LimitRanges() {
 ...
 const { data, isLoading, isError, refetch, pagination: hookPagination } = usePaginatedResourceList<LimitRangeResource>('limitranges');
 ...
 const toggleSelection = (r: LimitRangeRow, event?: React.MouseEvent) => {
 ...
 return executeBulkOperation(Array.from(selectedItems), async (_key, ns, name) => {
 await patchLR.mutateAsync({ name, namespace: ns, patch: { metadata: { labels: { [labelKey]: labelValue } } } });
 });
 };
 ...
 <ListPageStatCard label="Total Limit Ranges" value={stats.total} icon={Scale} iconColor="text-primary" selected={!hasActiveFilters} onClick={clearAllFilters} className={cn(!hasActiveFilters && !isLoading && 'ring-2 ring-primary')} isLoading={isLoading} />
 <ListPageStatCard label="Namespaces Covered" value={stats.namespacesCovered} icon={Scale} iconColor="text-muted-foreground" isLoading={isLoading} />
 <ListPageStatCard label="Container Limits" value={stats.containerLimits} icon={Scale} iconColor="text-blue-500" valueClassName="text-blue-500" selected={columnFilters.limitCategory?.size === 1 && columnFilters.limitCategory.has('Container')} onClick={() => setColumnFilter('limitCategory', new Set(['Container']))} className={cn(columnFilters.limitCategory?.size === 1 && columnFilters.limitCategory.has('Container') && 'ring-2 ring-blue-500')} isLoading={isLoading} />
 <ListPageStatCard label="Pod Limits" value={stats.podLimits} icon={Scale} iconColor="text-amber-600" valueClassName="text-amber-600" selected={columnFilters.limitCategory?.size === 1 && columnFilters.limitCategory.has('Pod')} onClick={() => setColumnFilter('limitCategory', new Set(['Pod']))} className={cn(columnFilters.limitCategory?.size === 1 && columnFilters.limitCategory.has('Pod') && 'ring-2 ring-amber-500')} isLoading={isLoading} />
 </div>

 <BulkActionBar
 selectedCount={selectedItems.size}
 resourceName="limit range"
 ...
 <ListPageLoadingShell columnCount={10} resourceName="limit ranges" isLoading={isLoading} onRetry={() => refetch()} />
 ) : isError ? (
 <TableRow>
